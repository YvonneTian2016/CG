...
// as of now only random sampling of quads.
inline vec3f sample_point(Surface* surface, vec2f uv)
{
    if(surface->isquad)
    {
        float side = surface->radius;
        vec3f left_corner = surface->frame.o - surface->frame.x * side - surface->frame.y * side;
        vec3f sampled_pos = (left_corner + surface->frame.x * uv.x * side * 2.0f )+ (surface->frame.y * uv.y * side * 2.0f);
        return sampled_pos;
    }
}

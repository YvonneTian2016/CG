...
float rand_num(){
    float random_x = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
    return random_x;
}
...
vec3f raytrace_ray(Scene* scene, const ray3f& ray, int depth) {
    ...
    auto kd = lookup_scaled_texture(intersection.mat->kd, intersection.mat->kd_txt, intersection.texcoord,intersection.mat->tile);
    ...
    // foreach surface
    for (Surface* sur :scene->surfaces){
        // skip if no emission from surface
        if(sur->mat->ke == zero3f) continue;
        // todo: pick a point on the surface, grabbing normal, area, and texcoord
        vec3f light_pos = zero3f;
        vec3f light_norm = zero3f;
        float area = 0.0f;
        vec2f random_num = zero2f;
        vec2f light_texcoord = zero2f;
        // check if quad
        if (sur->isquad){
            // generate a 2d random number
            float random_x = rand_num();
            float random_y = rand_num();
            random_num = vec2f(random_x,random_y);
            // compute light position, normal, area
            light_pos.x = (random_num.x-0.5)*sur->radius*2;
            light_pos.y = (random_num.y-0.5)*sur->radius*2;
            light_pos.z = 0.0;
            light_pos = transform_point(sur->frame,light_pos);
            light_norm = transform_normal(sur->frame,z3f);
            area = 4*sur->radius*sur->radius;
            // set tex coords as random value got before
            light_texcoord = random_num;
        }
        ...
        auto light_emission = lookup_scaled_texture(sur->mat->ke,sur->mat->ke_txt,light_texcoord);
        // compute light direction
        auto light_dir = normalize(light_pos-pos);
        // compute light response (ke * area * cos_of_light / dist^2)
        auto light_res = light_emission * area * max(0.0f,-dot(light_norm,light_dir)/lengthSqr(light_pos-pos));
        // compute the material response (brdf*cos)
        auto material_res = max(dot(norm,light_dir),0.0f) * eval_brdf(kd, ks, n, v, light_dir, norm, mf);
        // multiply brdf and light
        auto shade = material_res*light_res;
        // check for shadows and accumulate if needed
        if (shade == zero3f) continue;
        // if shadows are enabled
        if(scene->path_shadows) {
            // perform a shadow check and accumulate
            if(not intersect_shadow(scene,ray3f::make_segment(pos,light_pos)))
                c += shade;
        }
        else{
            // else just accumulate
    ...
    if(not (intersection.mat->kr == zero3f)) {
        ...
        auto r = 2*dot(norm,v)*norm - v;
        auto reflection_ray = ray3f(pos,r);
        // accumulate the reflected light (recursive call) scaled by the material reflection
        c += intersection.mat->kr*raytrace_ray(scene,reflection_ray,depth+1);
    }
